...
ORDER BY 
    total_questions_created DESC;
```
Now that's far more elegant isn't it?  Snowflake also implements a similar function they call `DIV()` which automatically returns 0 if there's a division by zero error
